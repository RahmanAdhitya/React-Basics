...
function App() {
  ...
  const renderContentList = () => {
    const jsxResult = data.map((val) => {
      return <ContentCard username={val.username} location={val.location} numberOfLikes={val.numberOfLikes} caption={val.caption} />;
    });
  ...
  const renderTodoList = () => {
    return todoList.map((val, idx) => {
      return <TodoItem date={val.date} action={val.action} isDone={val.isDone} deleteItem={() => deleteTodoItem(idx)} toggleStatus={() => toggleTodoStatus(idx)} />;
    });
  };
